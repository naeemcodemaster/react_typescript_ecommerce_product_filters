    ...
    return (
        <div className='w-64 p-4 h-screen'>
            ...
            <section>
                ...
                {/* Categories Section */}
                <div className='mb-5'>
                    <h2 className='text-xl font-semibold mb-3'>Categoires</h2>
                </div>
                <section>
                    {
                        categories.map((category, index) => (
                            <label key={index} className='block mb-2'>
                                <input type='radio' name='category' value={category} className='mr-2 w-[16px h-[16px]]' onChange={()=>handleRadioChangeCategory(category)} checked={selectedCategory === category}/>
                ...
                <div className="mb-5">
                    ...
                    <div>
                        {keywords.map((keyword, index) => (
                            <button key={index} onClick={()=>handleKeyword(keyword)} className='block mb-2 px-4 py-2 w-full text-left border rounded hover:bg-gray-200'>
                                {keyword.toLowerCase()}
                            </button>
                        ))}
                    </div>
                </div>
                <button onClick={handleReset} className='w-full mb-[4rem] py-2 bg-black text-white rounded mt-5'>Reset Filters</button>
            </section>
        </div>
    )
}
